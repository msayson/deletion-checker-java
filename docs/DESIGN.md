...
- ~~Benchmark prefix‑index bucket size `K`~~ — **done**, see [`benchmarks/bucket-size.md`](benchmarks/bucket-size.md) and [`benchmarks/analysis.md`](benchmarks/analysis.md) §5. Swept `{128, 256, 512, 1024, 2048, 4096}` × three identifier shapes × {1, 3, 5} entity types at 1M ids/type: `K = 128` is fastest or statistically tied everywhere, larger `K` is only slower (the widening in‑bucket search costs more than the shrinking separator array saves — the array already fits L2 at 128), and the index memory a larger `K` would save is negligible next to the identifier data. **Keep `K = 128`.** It is a fixed format default (`PrefixIndex.DEFAULT_BUCKET_SIZE`), not a generator CLI option; `BucketSizeBenchmarkTest` varies it directly for any future re‑tune.
