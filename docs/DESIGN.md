...
1. After sorting, slice the identifiers into buckets of a fixed target size `K = 128` (benchmark‑confirmed — [`benchmarks/bucket-size.md`](benchmarks/bucket-size.md), [`benchmarks/analysis.md`](benchmarks/analysis.md) §5): `bucketCount = max(1, ceil(identifierCount / K))`. This formula alone handles small datasets — when `identifierCount ≤ K`, `bucketCount` is already 1, so no separate small‑dataset threshold is needed.
...
- Zero‑padding a short identifier to a fixed width is only safe when the content can't contain a literal `0x00` byte (guaranteed for ASCII text, not for arbitrary UTF‑8). This only matters if a hypothetical future fixed‑width layout is ever adopted — the current variable‑length offset‑table format never relies on it.

### **5.7 Bloom Filter (v2)**
A per‑entity‑type **blocked Bloom filter**, built from the same identifier list at generation time and checked first on the lookup path (§6.2). It answers only "definitely not present" or "maybe present": a "maybe" falls through to the exact two‑level search, so the exactness guarantee of §5.6 is untouched; a "definitely not" is the common negative‑lookup case (most queries are for non‑deleted entities, §2) answered in one hash and one cache‑line read, without touching the Prefix Index or the Identifier Offset Table.
