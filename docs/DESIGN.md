...
Rationale for hand‑rolling the filter rather than adding a dependency (Guava et al.) is in `IMPLEMENTATION_PLAN.md` D14 — in short, every mature Bloom library is on‑heap, and a blocked filter is both smaller as a dependency and faster per probe.
...
- **Bloom filter frontend** (IMPLEMENTATION_PLAN B13) — a per‑entity‑type bit array, built from the same identifier list at generation time, checked before the two‑level binary search. It only ever answers "definitely not present" or "maybe present," so it accelerates the common negative‑lookup case (most queries are for non‑deleted entities) without weakening the exact‑match guarantee — a "maybe" still falls through to the real search. It does **not** help true‑positive latency (a positive pays the filter probe *and* the full search) or fix bucket skew — the adaptive prefix index (§5.4) is required regardless. **Blocked** (cache‑line‑sized blocks: one hash picks the block, `k` hashes test bits within it) so a probe is one cache‑line read, not `k` scattered ones — this is what makes it faster than the current negative path. It lives in the mmap'd file (off‑heap, ~1.5 bytes/identifier at a 1% target FPR) and is **hand‑rolled, not Guava**: every mature Bloom library is on‑heap, which would reintroduce the GC / container‑memory cost §4.1 removes, and a blocked filter is both smaller in dependency terms and faster per probe (IMPLEMENTATION_PLAN D14). Adds a new binary‑format section, so `formatVersion` → 2 (the runtime reads both), plus its own generation/validation step. Defer until production profiling shows negative‑lookup latency is a real bottleneck.
