...
Measured numbers for both — bytes/identifier, lookup latency, and load time across identifier shape, set size (1K–10M), and entity‑type count — are in [`benchmarks/reference.md`](benchmarks/reference.md) (interpreted in [`benchmarks/analysis.md`](benchmarks/analysis.md)), regenerated by the `benchmarks` module (`./gradlew :benchmarks:benchmark`). The shape of the result: for positive lookups `HashSet` wins raw latency at every size (a hash + `equals` beats an encode + two‑level byte‑wise binary search), but by well under a microsecond at the median and with both implementations comfortably sub‑millisecond at p99.9 (§2); for negative lookups — the common access‑control case — the v2 Bloom frontend (§5.7, [`benchmarks/analysis.md`](benchmarks/analysis.md) §6) closes that gap, dropping the packed negative p50 ~3–5x to level with `HashSet`. What the packed set trades that for is decisive at fleet scale: ~100–125 heap bytes/identifier for `HashSet<String>` versus a near‑zero heap footprint (the identifiers live in the mmap'd file, off the Java heap, ~20–40 bytes/identifier of reclaimable page cache), no GC pressure from millions of resident objects, and a load that is tens of milliseconds rather than the ~1 s it takes to populate a 10M‑entry `HashSet`.
