...
### **5.5 Checksum**
Each entity‑type file's checksum is the **xxHash64** hash of the complete file with the checksum field's own bytes replaced by zero before hashing. This resolves the self‑reference problem (a checksum can't include its own final value) while covering every other byte in the file — including header fields like `entityType`, `identifierCount`, `bucketSize`, and `bucketCount`, not just the Prefix Index / Offset Table / Identifier Data — so header corruption is caught too, not only payload corruption. Verification re‑derives the hash the same way and compares. This can be implemented by streaming three logical segments (bytes before the checksum field, zero‑padding in its place, then bytes after) without ever materializing a modified copy of the file.

xxHash64 is chosen to prioritize speed during both dataset generation and startup validation; it is not cryptographically secure, which is an accepted tradeoff given the assumed threat model — this guards against truncation/corruption, not against a malicious actor deliberately crafting a colliding file.

The manifest's own checksum is **SHA256** instead (§5.1), rather than xxHash64 — it's small and read once, so the slower per‑byte cost is irrelevant, and it's the trust anchor pinning every entity‑type file's expected xxHash64 checksum, so it's worth the stronger guarantee. The whole requested set can be validated for consistency (manifest checksum, then each loaded file's checksum) without merging any identifier data.

SHA-256 and xxHash provide corruption detection, not artifact authenticity.
